...
- No. I completed the project independently.
...
2. Did you use LLMs or other AI models to generate text or code for this project? If so, which tools did you use and how did you use them? Remember in this class you *are* allowed (even encouraged) to use these tools for help with coding, but not for writing text. This can be very brief, just one or two sentences is fine unless you have more you'd like to add.
    1. I used ChatGPT to help me debug and understand some areas that I am not sure if I coded correctly
...
1.  use frequent, informative comments in code and markdown (demonstrated)
2.  find and understand documentation for R packages and functions (demonstrated)
...
9.  create and use intermediate datasets (demonstrated)
...
12. name and differentiate R data types (demonstrated)
13. name and differentiate R objects (demonstrated)
...
1.  use frequent, informative comments in code and markdown
 - My code contains clear, structured comments explaining each step.
2.  find and understand documentation for R packages and functions 
 - I referenced '?mtcars' and used daff::diff_data() for debugging.
3.  follow a debugging workflow with independent and collaborative strategies
 - I used all.equal(), diff_data(), and validated fixes step by step. 
4.  parse and investigate error messages
 - Used debugging methods to identify mismatches in goal.mtcars.
5.  follow a coding style guide
 - I followed tidyverse style (pipes, indentation, function organization).
9.  create and use intermediate datasets 
 - I created clean.mtcars and auto.cars as intermediate datasets.
12. name and differentiate R data types 
 - I correctly converted variables to numeric, logical, and factor types.
13. name and differentiate R objects 
 - I worked with data frames (tibbles), factors, and numeric vectors.
...
This project was a helpful experience in applying tidyverse functions for data cleaning, transformation, and debugging. I gained a deeper understanding of renaming, reordering, and converting data types, as well as ensuring the cleaned dataset matched ‘goal.mtcars.’ One of the biggest takeaways was the importance of systematic debugging—using all.equal() and diff_data() helped identify subtle discrepancies. 
The key challenge was  was achieving a perfect match with goal.mtcars, which required precise data corrections, including fixing typos and rounding numbers correctly. 
The most rewarding part was seeing all the transformations come together to create a cleaned dataset that matched ‘goal.mtcars.’ This project significantly strengthened my confidence in dplyr, stringr, and forcats, reinforcing how thorough data preparation is crucial for accurate analysis. 
